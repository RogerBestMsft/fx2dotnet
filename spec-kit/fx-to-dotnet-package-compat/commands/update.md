...
- After each chunk, invoke `speckit.fx-to-dotnet-build-fix.fix` and evaluate outcome before proceeding
...
3. Invoke `speckit.fx-to-dotnet-build-fix.fix` on the same solution/project target
